...
namespace MyMath {
	...
	static const uint32_t FAST_MATH_TABLE_SIZE = 512;
	...
	template<typename T> void saturate(T &x, T a){
		if(x >  a) x=a;
		if(x < -a) x=-a;
	}

	template<typename T> T saturated(T x, T a){
		T y=x; saturate(y,a);
		return y;
	}

	template<typename T> void normalize(T &x, T a){
		while(x>a){ x-=T(2)*a; }
		while(x<=-a){ x+=T(2)*a; }
	}

	template<typename T> T normalized(T x, T a){
		T y=x; normalize(y,a);
		return y;
	}
